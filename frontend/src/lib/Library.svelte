<script lang="ts">
  ...
  let showDeleteConfirm = false; // New state for delete dialog
  ...
  function closeBook() { 
    selectedBook = null; 
    showDeleteConfirm = false; // Ensure confirm is closed
  }
  ...
  <header class="lib-header">
    ...
    <div class="filters">
      {#each ['All', 'Reading', 'To Read', 'Finished'] as filter}
        <button class="filter-pill" class:active={activeFilter === filter} on:click={() => setFilter(filter)}>{filter}</button>
      {/each}
    </div>
  </header>
  <div class="book-grid">
    {#each sortedBooks as book (book.id)}
      <div class="book-card" on:click={() => openBook(book)} on:keydown={(e) => e.key === 'Enter' && openBook(book)} tabindex="0" role="button" animate:flip={{ duration: 300 }}>
...
{#if selectedBook}
  ...
  >
    <div class="modal-card" role="dialog" aria-modal="true" transition:scale={{ duration: 300, easing: cubicOut, start: 0.95 }}>
        ...
        <div class="status-actions">
          ...
          <div class="action-buttons">
            {#each ['Reading', 'To Read', 'Finished'] as status}
              <button class="status-btn {status.toLowerCase().replace(' ', '-')}" class:active={selectedBook.status === status} on:click={() => updateStatus(status)}>{status}</button>
            {/each}
          </div>
        </div>
      ...
      <!-- CUSTOM DELETE CONFIRMATION OVERLAY (Nested inside Modal) -->
...
<style>
  ...
  /* -- CUSTOM DELETE CONFIRMATION STYLES -- */
  .delete-confirm-overlay {
    position: absolute; 
    top: 0; left: 0; right: 0; bottom: 0;
    background: rgba(255, 234, 219, 0.685); /* Warm translucent overlay */
    backdrop-filter: blur(5px);
    display: flex; justify-content: center; align-items: center;
    z-index: 50;
    border-radius: 24px; /* Matches modal-card */
  }
