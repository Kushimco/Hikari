<script lang="ts">
  ...
  // Define local type or import from central types file if you have one
  type Book = {
    id: string;
    title: string;
    author: string;
  ...
  function updateProgress(delta: number) {
    ...
    // Logic to auto-update status based on progress
  ...
  function handleInput(e: Event) {
    ...
    // Reuse updateProgress logic but calculate delta relative to current
    // Or just emit directly since we have the absolute value
    let newStatus = book.status;
    if (book.total_pages > 0 && val >= book.total_pages) {
      newStatus = "finished";
...
<style>
  ...
  .progress-edit-wrapper { 
    ...
    /* Reset button default styles if it's a button */
    font-family: inherit;
    font-size: inherit;
    color: inherit;
  }
  ...
  .modal-progress-track { height: 8px; width: 100%; background: rgba(255,255,255,0.4); border-radius: 4px; overflow: hidden; margin-bottom: 8px; }
  .modal-progress-fill { height: 100%; background: #5e4b4b; transition: width 0.3s ease; }
  .progress-controls { display: flex; gap: 6px; }
  .prog-btn { background: rgba(255,255,255,0.4); border: 1px solid rgba(255,255,255,0.6); border-radius: 6px; padding: 4px 10px; font-size: 0.75rem; color: #5e4b4b; cursor: pointer; font-weight: 600; transition: all 0.2s; }
