...
async fn add_book(
    ...
    if books
        .iter()
        .any(|b| b.title.eq_ignore_ascii_case(&title) && b.author.eq_ignore_ascii_case(&author))
    {
    ...
    let cover_path =
        task::spawn_blocking(move || match download_cover_image(&app_clone, &cover_url) {
...
async fn backup_library(app: tauri::AppHandle, target_path: String) -> Result<(), String> {
    ...
    let backup_items: Vec<BackupItem> = books
        .into_iter()
        .map(|b| {
...
async fn restore_library(app: tauri::AppHandle, source_path: String) -> Result<(), String> {
    ...
    let books: Vec<Book> = match serde_json::from_str::<Vec<BackupItem>>(&content) {
        ...
        Err(_) => {
            ...
            serde_json::from_str::<Vec<Book>>(&content)
